  ...
  const timeoutId = setTimeout(async () => {
    ...
    if (day !== 0 && day !== 6) {
      await sendToChat(
        ...
        `📢 **Напоминание по код-ревью:**
        2 аппрува = **ты мержишь свой MR** ✅
        Создал тред = *ты его и решаешь (зарезолвить)* 💬
        Всем спасибо! 🚀`,
        chatId
      );
    }
 ...
 * Команда: /show_ex
 */
bot.command('show_ex', (ctx) => {
  let response = `⏰ Через 10 минут начинается ${
      windowKey === 'morning' ? 'утреннее' : 'вечернее'
    } окно PR (${timeWindow})!\n`
    `📢 **Напоминание по код-ревью:**
    2 аппрува = **ты мержишь свой MR** ✅
